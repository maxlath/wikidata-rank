const db = require('./db')
const logAndExit = require('./log_and_exit')
const { green } = require('chalk')
const { pid } = require('./status')
const fs = require('fs')
const importFile = fs.createWriteStream('./leveldb_pending_import', { flags: 'a' })
const addToImportFile = Promise.promisify(importFile.write, { context: importFile })

var ops = []
var count = 0
var totalOps = 0

// Randomizing batch length so that workers don't all try to put
// at the same time
const batchLength = 1000 + Math.trunc(Math.random() * 1000)

module.exports = {
  aggregate: ({ newOps, stream, flush }) => {
    if (newOps instanceof Array) {
      ops.push(...newOps)
    } else {
      ops.push(newOps)
    }

    // Aggregate operations before puting in batch
    if (!flush && ops.length < batchLength) return

    count += 1

    const key = `${pid} batch ${count}`

    if (stream) stream.pause()

    totalOps += ops.length

    return add(ops)
    .catch(logAndExit('aggregated batch'))
  },

  flush: () => {
    totalOps += ops.length
    return add(ops)
    .then(() => {
      console.log(pid, green(`total key/value pairs added:`), totalOps)
    })
    .catch(logAndExit('flush batch'))
  }
}


// MAKES WAAAAY TO HEAVY FILES
const add = ops => {
  const stringifiedOps = ops
    .map(op => JSON.stringify({ key: op.key, value: op.value }))
    .join('\n')
  return addToImportFile(stringifiedOps + '\n')
}